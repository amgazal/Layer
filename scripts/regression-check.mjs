...
const checks = [
  ...
  ["signing in automatically enables account sync without anonymous pre-auth", /Sign in to sync your profile/.test(source) && /Signing in turns on account sync automatically/.test(source) && /intent === "signin" \? true : await prepareCloud\(\)/.test(source) && /mode !== "signin" && !cloudAllowed\(\)/.test(sync)],
  ...
  ["email callback can hand auth back to the original Layer tab", /layer-auth-handoff-v1/.test(source) && /layer-auth-handoff-v1/.test(fs.readFileSync(new URL("../public/auth-callback.html", import.meta.url), "utf8"))],
  ...
  ["returning-user restore suppresses onboarding flicker", /auth\.status === "checking"/.test(source) && /accountRestoreBusy/.test(source) && /Loading your saved Layer profile/.test(source)],
  ...
  ["permanent account sessions force account sync on", /permanent && cloudPreference\(\) !== "on"/.test(sync)],
  ["additional user-shot campus scenes are wired in", /clear-campus\.webp/.test(source) && /partly-campus\.webp/.test(source) && /sunset-campus\.webp/.test(source)],
